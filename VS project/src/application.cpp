...
Application::Application(int window_width, int window_height, SDL_Window* window)
{
	...
	Volume* vol1 = new Volume(32, 32, 32);
	Volume* vol2 = new Volume(32, 32, 32);
	Volume* vol3 = new Volume(32, 32, 32);
	vol2->loadPVM("data/volumes/Orange.pvm");
	vol3->loadVL("data/volumes/texture3d.vl");

	//----------------------new
	vol1->loadPVM("data/volumes/CT-Abdomen.pvm");
	tex1 = new Texture();
	tex2 = new Texture();
	tex3 = new Texture();

	tex1->create3D(vol1->width, vol1->height, vol1->depth, GL_RED, GL_UNSIGNED_BYTE, false, vol1->data, GL_RED);
	tex2->create3D(vol2->width, vol2->height, vol2->depth, GL_RED, GL_UNSIGNED_BYTE, false, vol2->data, GL_RED);
	tex3->create3D(vol3->width, vol3->height, vol3->depth, GL_RED, GL_UNSIGNED_BYTE, false, vol3->data, GL_RED);

	material->texture = tex2;
	//-----------------------

	// Manipulate material
	//material->color = vec4(1.0, 0.0, 0.0, 1.0);
	//material->texture = Texture::Get("data/textures/texture.tga");
	//material->texture = Texture::Get("data/volumes/texture3d.vl");
	//material->texture->create3D(512, 512, 512);

	material->shader = Shader::Get("data/shaders/basic.vs", "data/shaders/raytracing.fs");
...
void Application::update(double seconds_elapsed)
{
	...
	StandardMaterial* mat = (StandardMaterial*)root[0]->material;
	int aux = mat->text_num;
	switch (aux) {
	default:
	case 0:
		root[0]->material->texture = tex1;
		break;
	case 1:
		root[0]->material->texture = tex2; 
		break;
	case 2:
		root[0]->material->texture = tex3;
		break;
